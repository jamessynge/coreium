#ifndef MCUCORE_SRC_STRINGS_TINY_STRING_H_
#define MCUCORE_SRC_STRINGS_TINY_STRING_H_
...
class TinyString {
 public:
  ...
  bool Set(const char* from, size_type size) {
    MCU_DCHECK_LE(size, N) << MCU_PSD("Too big");
    Clear();
    if (size > N) {
      return false;
    }
...
#endif  // MCUCORE_SRC_STRINGS_TINY_STRING_H_
