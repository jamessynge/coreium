...
TEST_F(McuVLogTest, VLogLevelIsEnabled) {
  ...
  MCU_VLOG(1) << "one";
  EXPECT_THAT(TakeStr(),
              absl::StrCat("log_test.cc:", line_before + 1, "] one\n"));

  line_before = __LINE__;
  MCU_VLOG(2) << "two";
  EXPECT_THAT(TakeStr(),
              absl::StrCat("log_test.cc:", line_before + 1, "] two\n"));

  line_before = __LINE__;
  MCU_VLOG(3) << "three";
  EXPECT_THAT(TakeStr(),
              absl::StrCat("log_test.cc:", line_before + 1, "] three\n"));

  line_before = __LINE__;
  MCU_VLOG(4) << "four";
  EXPECT_THAT(TakeStr(),
              absl::StrCat("log_test.cc:", line_before + 1, "] four\n"));

  line_before = __LINE__;
  MCU_VLOG(5) << "five";
  EXPECT_THAT(TakeStr(),
              absl::StrCat("log_test.cc:", line_before + 1, "] five\n"));
}
...
TEST_F(McuVLogTest, NoMessage) {
  ...
  int line_before = __LINE__;
  MCU_VLOG(1);
  EXPECT_THAT(TakeStr(), absl::StrCat("log_test.cc:", line_before + 1, "] \n"));
}
...
TEST_F(McuVLogTest, MiscValues) {
  ...
  EXPECT_THAT(TakeStr(),
              absl::StrCat("log_test.cc:", line_before + 1, "] abc\n"));
}
