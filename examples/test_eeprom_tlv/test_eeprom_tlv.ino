...
void loop() {
  ...
  LogSink() << MCU_FLASHSTR_128(
                   "\n----------------------------------------"
                   "----------------------------------------\n")
            << MCU_PSD("EepromTlv::GetOrDie");
  auto tlv = EepromTlv::GetOrDie();
  LogSink() << MCU_PSD("got");
  MCU_CHECK_OK(tlv.Validate());
  LogSink() << MCU_PSD("validated");
  LogSink() << tlv << '\n';

  SomeData some_data{.f = 99999};
  MCU_CHECK_OK(FillFromTlv(tlv, some_data));
  LogSink() << MCU_PSD("found f=") << some_data.f;

  some_data.f += 1.01;
  LogSink() << MCU_PSD("writing f=") << some_data.f;
